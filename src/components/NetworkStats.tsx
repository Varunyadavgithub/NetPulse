...
export const NetworkStats: React.FC<NetworkStatsProps> = ({ stats }) => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <Card key={index} className="animate-slide-up" style={{ animationDelay: `${index * 100}ms` }}>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-dark-600">{stat.title}</p>
      ...
      <Card className="md:col-span-2 lg:col-span-4 animate-slide-up" style={{ animationDelay: '400ms' }}>
        <CardHeader>
          <h3 className="text-lg font-semibold text-dark-900">Network Health</h3>
        </CardHeader>
        <CardContent>
          <div className="flex items-center space-x-4">
            <div className="flex-1">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm font-medium text-dark-700">Overall Uptime</span>
                <span className="text-sm font-bold text-dark-900">{uptime.toFixed(1)}%</span>
              </div>
            ...
            <div className={`px-3 py-1 rounded-full text-sm font-medium ${
              uptime >= 95 ? 'bg-success-100 text-success-800' :
              uptime >= 80 ? 'bg-warning-100 text-warning-800' :
